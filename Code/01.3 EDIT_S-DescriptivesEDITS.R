# ==============================================================================
# EDIT Servicios - Descriptivos y graficos en R
# ==============================================================================
...
extract_year_from_label <- function(label_clean, year_start, year_end) {
  ...
  ifelse(!is.na(year_hit), year_hit, suppressWarnings(as.integer(year_end)))
}

classify_indicator <- function(variable_panel, label_clean) {
  case_when(
    str_detect(label_clean, "total personal ocupado promedio") &
      !str_detect(label_clean, "participo|participó|acti|educacion superior") ~ "empleo_total",
    str_detect(label_clean, "total personal ocupado promedio") &
      str_detect(label_clean, "acti") &
      str_detect(label_clean, "participo|participó") ~ "empleo_acti",
    str_detect(label_clean, "ventas nacionales totales|ingresos o ventas nacionales totales|ventas totales|ingresos operacionales") ~ "ventas_nacionales",
    str_detect(label_clean, "actividades de i\\+d internas|i\\+d internas|investigacion y desarrollo i \\+ d") &
      str_detect(label_clean, "monto invertido|total inversion|total inversión|miles de pesos") ~ "inversion_id_interna",
    str_detect(label_clean, "adquisicion de maquinaria y equipo|maquinaria y equipo") &
      str_detect(label_clean, "monto invertido|total inversion|total inversión|miles de pesos") ~ "inversion_maquinaria_equipo",
    str_detect(label_clean, "numero total de innovaciones|número total de innovaciones|total innovaciones") ~ "numero_innovaciones",
    str_detect(label_clean, "si=1, no=2|si =1, no=2|si=1 no=2") &
      str_detect(label_clean, "bienes o servicios nuevos|bienes o servicios mejorados|metodos de produccion|metodos de prestacion|metodos organizativos|tecnicas de comercializacion|sistemas logisticos|distribucion") ~ "indicador_innova",
    TRUE ~ NA_character_
  )
}
...
detect_department <- function(label_clean) {
  departamentos <- c(
    "amazonas", "antioquia", "arauca", "atlantico", "bogota d.c.", "bogota",
...
panel_path <- file.path(processed_dir, "EDITS_Panel.dta")
dictionary_path <- file.path(processed_dir, "EDITS_Panel_diccionario_variables.xlsx")
...
if (!file.exists(dictionary_path)) stop("No existe: ", dictionary_path)
...
dict_raw <- read_excel(dictionary_path, sheet = "diccionario_panel", col_types = "text") %>%
  mutate(
...
dict <- dict_raw %>%
  ...
  distinct(edit_period, indicador_year, indicador, variable_panel, label_dane, .keep_all = TRUE)
...
dept_dict <- dict_raw %>%
  ...
  distinct(edit_period, indicador_year, departamento, variable_panel, label_dane, .keep_all = TRUE)
...
metric_availability <- base %>%
  ...
  summarise(
    empresas = n(),
    across(
      c(empleo_total, empleo_acti, ventas_nacionales, inversion_id_interna, inversion_maquinaria_equipo, numero_innovaciones, firma_con_innovacion),
      ~ sum(!is.na(.x)),
      .names = "n_{.col}"
    ),
    .groups = "drop"
...
plot_empresas_sector <- sector_year %>%
  ...
  labs(title = "EDIT-S: empresas por division CIIU Rev. 4", x = NULL, y = "Empresas", color = "Sector") +
  theme_edit
...
plot_empleo_sector <- sector_year %>%
  ...
  geom_point(size = 1.6) +
  scale_y_continuous(labels = comma) +
  labs(title = "EDIT-S: personal ocupado promedio por division CIIU Rev. 4", x = NULL, y = "Personal ocupado", color = "Sector") +
  theme_edit
...
plot_ventas_trabajador <- sector_year %>%
  ...
  geom_point(size = 1.6) +
  scale_y_continuous(labels = comma) +
  labs(title = "EDIT-S: ventas o ingresos por trabajador", x = NULL, y = "Miles de pesos corrientes por trabajador", color = "Sector") +
  theme_edit
...
plot_inversion_id <- sector_year %>%
  ...
  geom_point(size = 1.6) +
  scale_y_continuous(labels = comma) +
  labs(title = "EDIT-S: inversion en I+D interna por trabajador", x = NULL, y = "Miles de pesos corrientes por trabajador", color = "Sector") +
  theme_edit
...
plot_intensidad_acti <- sector_year %>%
  ...
  labs(title = "EDIT-S: intensidad de personal ACTI", x = NULL, y = "Personal ACTI / personal ocupado", color = "Sector") +
  theme_edit

latest_dept_year <- if (nrow(dept_year) > 0) max(dept_year$year, na.rm = TRUE) else NA_integer_

plot_dept_acti <- if (!is.na(latest_dept_year)) {
  dept_year %>%
    ...
    coord_flip() +
    scale_y_continuous(labels = comma) +
    labs(title = paste0("EDIT-S: personal ACTI por departamento, ", latest_dept_year), x = NULL, y = "Personal ACTI") +
    theme_edit
} else {
  NULL
}
...
if (!is.null(plot_dept_acti)) print(plot_dept_acti)

message("Listo. Los objetos base, sector_year y dept_year quedan disponibles en memoria.")
