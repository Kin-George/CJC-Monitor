# ==============================================================================
# EDIT - Construccion de panel consolidado
# ==============================================================================
# Fuente: Datos/Raw/EDIT
# Salida: Datos/Processed/EDIT_Panel.dta
#
# Notas:
# - Las rondas de EDIT cambian nombres, mayusculas/minusculas y estructura.
# - Se conserva el cuestionario completo con nombres armonizados, en vez de
#   seleccionar manualmente variables por codigo de pregunta.
# - Se crea CIIU homologado a Rev. 4 A.C. usando la tabla correlativa DANE
#   TC-CIIU-3ACvsCIIU-4AC-2020.xlsx.
# - En bases donde solo existe CIIU a 3 digitos, la homologacion queda a nivel
#   de grupo; se marca en ciiu4_nivel_homologacion.
# ==============================================================================
  ...
  repeat {
    if (
      dir.exists(file.path(current, "Code")) &&
        dir.exists(file.path(current, "Datos"))
    ) {
      return(current)
    }
...
output_path <- file.path(processed_dir, "EDIT_Panel.dta")
audit_path <- file.path(processed_dir, "EDIT_Panel_auditoria_archivos.csv")
vars_path <- file.path(processed_dir, "EDIT_Panel_diccionario_variables.csv")
vars_xlsx_path <- file.path(processed_dir, "EDIT_Panel_diccionario_variables.xlsx")
ciiu_audit_path <- file.path(processed_dir, "EDIT_Panel_auditoria_ciiu.csv")

if (!dir.exists(raw_dir)) {
  stop("No existe la carpeta de datos EDIT: ", raw_dir)
}
if (!file.exists(correlativa_path)) {
  stop("No existe la tabla correlativa CIIU: ", correlativa_path)
}
if (!file.exists(dictionary_path)) {
  stop("No existe el diccionario consolidado EDIT: ", dictionary_path)
}
...
extract_years <- function(file_name) {
  ...
  yrs <- as.integer(yrs)
  if (length(yrs) >= 2) {
    return(list(start = min(yrs), end = max(yrs), period = paste0(min(yrs), "_", max(yrs))))
  }
  if (length(yrs) == 1) {
    return(list(start = yrs[1], end = yrs[1], period = as.character(yrs[1])))
  }
...
num_from_text <- function(x, width = NULL) {
  ...
  if (!is.null(width)) {
    out <- ifelse(is.na(out), NA_character_, str_pad(out, width = width, pad = "0"))
  }
  out
}
...
build_ciiu_bridge <- function(path) {
  ...
  class_map <- bridge %>%
    ...
    group_by(rev3_class) %>%
    summarise(
      ciiu4_hom = modal_first(rev4_class),
      n_destinos_clase = n_distinct(rev4_class),
      .groups = "drop"
    )
  ...
  group_map <- bridge %>%
    ...
    group_by(rev3_group) %>%
    summarise(
      ciiu4_group_hom = modal_first(rev4_group),
      n_destinos_grupo = n_distinct(rev4_group),
      .groups = "drop"
    )
  ...
  div_map <- bridge %>%
    ...
    group_by(rev3_div) %>%
    summarise(
      ciiu4_div_hom = modal_first(rev4_div),
      n_destinos_div = n_distinct(rev4_div),
      .groups = "drop"
    )

  list(class = class_map, group = group_map, div = div_map)
}

ciiu_bridge <- build_ciiu_bridge(correlativa_path)

load_edit_dictionary <- function(path) {
...
edit_dictionary <- load_edit_dictionary(dictionary_path)
...
as_plain_character <- function(x) {
  if (inherits(x, "haven_labelled")) {
    return(as.character(as_factor(x, levels = "values")))
  }
  as.character(x)
}
...
detect_ciiu <- function(data, year_start, year_end) {
  nms <- names(data)
  out <- list(
    var = NA_character_,
    rev = NA_integer_
  ...
  if ("CIIU4" %in% nms) {
    out$var <- "CIIU4"; out$rev <- 4L
  } else if ("CIIU_4" %in% nms) {
    out$var <- "CIIU_4"; out$rev <- 4L
  } else if ("CIIU3" %in% nms) {
    out$var <- "CIIU3"; out$rev <- 3L
  } else if ("CIIU_3" %in% nms) {
    out$var <- "CIIU_3"; out$rev <- 3L
  } else if ("ACT3" %in% nms) {
    out$var <- "ACT3"; out$rev <- 3L
  } else if ("ACT" %in% nms) {
    out$var <- "ACT"; out$rev <- 3L
  }
...
add_ciiu_homologation <- function(data, bridge) {
  data <- data %>%
    mutate(
      ...
      ciiu3_class = case_when(
        ciiu_revision_original == 3L & ciiu_digits >= 4 ~ str_sub(str_pad(ciiu_original, 4, pad = "0"), 1, 4),
        TRUE ~ NA_character_
      ),
      ciiu3_group = case_when(
        ciiu_revision_original == 3L & ciiu_digits >= 3 ~ str_sub(str_pad(ciiu_original, 3, pad = "0"), 1, 3),
        TRUE ~ NA_character_
      ),
      ciiu3_div = case_when(
        ciiu_revision_original == 3L & ciiu_digits >= 2 ~ str_sub(str_pad(ciiu_original, 2, pad = "0"), 1, 2),
        TRUE ~ NA_character_
      ),
      ciiu4_class_direct = case_when(
        ciiu_revision_original == 4L & ciiu_digits >= 4 ~ str_sub(str_pad(ciiu_original, 4, pad = "0"), 1, 4),
        TRUE ~ NA_character_
      ),
    ...
    mutate(
      ciiu4_homologado = case_when(
        !is.na(ciiu4_class_direct) ~ ciiu4_class_direct,
        !is.na(ciiu4_group_direct) ~ ciiu4_group_direct,
        !is.na(ciiu4_div_direct) ~ ciiu4_div_direct,
        !is.na(ciiu4_hom) ~ ciiu4_hom,
        !is.na(ciiu4_group_hom) ~ ciiu4_group_hom,
        !is.na(ciiu4_div_hom) ~ ciiu4_div_hom,
        TRUE ~ NA_character_
      ),
      ...
      ciiu4_div = case_when(
        ciiu4_nivel_homologacion %in% c("clase_rev4_directa", "clase_rev3_a_rev4") ~ str_sub(ciiu4_homologado, 1, 2),
        ciiu4_nivel_homologacion %in% c("grupo_rev4_directo", "grupo_rev3_a_rev4") ~ str_sub(ciiu4_homologado, 1, 2),
    ...
    select(
      -ciiu4_class_direct, -ciiu4_group_direct, -ciiu4_div_direct,
      -ciiu4_hom, -ciiu4_group_hom, -ciiu4_div_hom,
      -n_destinos_clase, -n_destinos_grupo, -n_destinos_div
    )
...
process_edit_file <- function(path) {
  ...
  ciiu_info <- detect_ciiu(df, years$start, years$end)

  if (!is.na(ciiu_info$var)) {
    ciiu_original <- as_plain_character(df[[ciiu_info$var]])
  } else {
    ciiu_original <- rep(NA_character_, nrow(df))
  }

  tipologia_var <- intersect(c("TIPOLOGIA", "TIPOLO", "TIPOLO2004", "TIPOLO1996"), names(df))
...
if (length(edit_files) == 0) {
  stop("No encontre archivos .dta en: ", raw_dir)
}
...
for (col in all_columns) {
  is_char_by_col[[col]] <- any(map_lgl(data_list, function(df) {
    col %in% names(df) && (is.character(df[[col]]) || is.factor(df[[col]]))
  }))
}
...
ciiu_audit <- panel %>%
  group_by(
    ...
    year_start,
    year_end,
    ciiu_revision_original,
    ciiu_var_original,
    ciiu4_nivel_homologacion,
    ciiu4_homologacion_ambigua
  ) %>%
  summarise(
    observaciones = n(),
...
panel <- panel %>%
  select(any_of(ordered_first), everything())
...
for (i in seq_len(nrow(panel_variable_labels))) {
  ...
  if (var_i %in% names(panel)) {
    attr(panel[[var_i]], "label") <- substr(panel_variable_labels$label_panel[[i]], 1, 80)
  }
}
...
message("Listo.")
message("Panel EDIT: ", output_path)
